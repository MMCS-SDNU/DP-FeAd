...
from utils import get_dataset, calc_map_k, split_data, agg_func, proto_aggregation,CalcTopMapWithPR
...
def FedProto_taskheter(opt, X, Y, L, user_groups, img_model, txt_model, img_discrimnator, txt_discrimnator, S, local_F_buffer_dict,
                       ...
                       img_discrimnator_params_list, txt_discrimnator_params_list,
                       img_discrimnator_params_name, txt_discrimnator_params_name):
    ...
    img_discrim_hypernet = ImageHyperNetwork(
        modal='img_discrim',
        embedding_dim=opt.embedding_dim_img,
        client_num=opt.num_users,
        hidden_dim=opt.hidden_dim_img,
        backbone=img_discrimnator,
        gpu=opt.use_gpu,
    )
    txt_discrim_hypernet = TextHyperNetwork(
        modal='txt_discrim',
        embedding_dim=opt.embedding_dim_txt,
        client_num=opt.num_users,
        hidden_dim=opt.hidden_dim_txt,
        backbone=txt_discrimnator,
        gpu=opt.use_gpu,
    )

    # fedproto begin
    clustered_img_protos, unbiased_img_protos, clustered_txt_protos, unbiased_txt_protos = [], [], [], []
    idxs_users = np.arange(opt.num_users)

    for round in range(opt.max_epoch):
        local_losses, local_proto_losses, local_adversarial_losses = [], [], []
        local_img_protos, local_txt_protos = {}, {}
        print(f'\n | Global Training Round : {round + 1} |\n')

        # proto_img_loss, proto_txt_loss = 0, 0
        client_class_counts = np.zeros((opt.num_users, opt.num_class), dtype=int)
        for idx in idxs_users:
            ...
            img_discrim_layer_params_dict = dict(
                zip(img_discrimnator_params_name, list(zip(*img_discrimnator_params_list))))
            txt_discrim_layer_params_dict = dict(
                zip(txt_discrimnator_params_name, list(zip(*txt_discrimnator_params_list))))
            img_discrim_alpha = img_discrim_hypernet(idx)
            txt_discrim_alpha = txt_discrim_hypernet(idx)
            img_discrim_aggregated_parameters, txt_discrim_aggregated_parameters = {}, {}
            for name in img_discrimnator_params_name:
                a = img_discrim_alpha[name.split(".")[-2]]
                img_discrim_aggregated_parameters[name] = torch.sum(
                    a
                    / a.sum()
                    * torch.stack(img_discrim_layer_params_dict[name], dim=-1).cuda(),
                    dim=-1,
                )
            img_discrimnator_params_list[idx] = list(img_discrim_aggregated_parameters.values())
            for name in txt_discrimnator_params_name:
                a = txt_discrim_alpha[name.split(".")[-2]]
                txt_discrim_aggregated_parameters[name] = torch.sum(
                    a
                    / a.sum()
                    * torch.stack(txt_discrim_layer_params_dict[name], dim=-1).cuda(),
                    dim=-1,
                )
            txt_discrimnator_params_list[idx] = list(txt_discrim_aggregated_parameters.values())
            ...
            img_discrim_net = copy.deepcopy(img_discrimnator)
            img_discrim_net.load_state_dict(img_discrim_aggregated_parameters, strict=True)
            txt_discrim_net = copy.deepcopy(txt_discrimnator)
            txt_discrim_net.load_state_dict(txt_discrim_aggregated_parameters, strict=True)
            ...
            for labels in local_train_L:
                for j, label in enumerate(labels):
                    if label == 1:
                        client_class_counts[idx][j] += 1

            epoch_loss = {'total': [], 'proto': [], 'adversarial_loss': []}
            epoch_weights = {'image': img_model_net.state_dict(),
                             'text': txt_model_net.state_dict(),
                             'image_discrim': img_discrim_net.state_dict(),
                             'text_discrim': txt_discrim_net.state_dict()
                             }
            ...
            img_discrim_frz_model_params = OrderedDict({
                name: param.clone().detach().requires_grad_(param.requires_grad)
                for name, param in epoch_weights['image_discrim'].items()})
            txt_discrim_frz_model_params = OrderedDict({
                name: param.clone().detach().requires_grad_(param.requires_grad)
                for name, param in epoch_weights['text_discrim'].items()})
            ...
            for iter in range(opt.train_ep):
                ...
                img_discrim_net.load_state_dict(epoch_weights['image_discrim'])
                txt_discrim_net.load_state_dict(epoch_weights['text_discrim'])
                ...
                img_w, img_D, local_F_buffer, img_protos, loss_img_proto_loss, img_adversarial_loss = local_img_model.update_weights_het(
                    unbiased_img_protos, clustered_img_protos, img_discrim_net, model=img_model_net)
                ...
                local_txt_model = LocalTxtUpdate(opt=opt, G_buffer=local_G_buffer_dict[idx], Y=local_Y_dict[idx],
                                                 ...
                                                 train_label=local_train_L)
                txt_w, txt_D, local_G_buffer, txt_protos, loss_txt_proto_loss, txt_adversarial_loss = local_txt_model.update_weights_het(
                    unbiased_txt_protos, clustered_txt_protos, txt_discrim_net, model=txt_model_net)
                ...
                loss, loss_proto, adversarial_loss = calc_loss(local_B, local_F_buffer, local_G_buffer, local_Y_dict[idx], local_train_L,
                                             loss_img_proto_loss, loss_txt_proto_loss, opt,
                                             img_adversarial_loss , txt_adversarial_loss)
                print(
                    'Local loss...global round: %3d, client: %3d, local epoch: %3d, loss: %3.3f, proto loss: %3.3f, adversarial_loss: %3.3f,comment: update B'
                    % (round + 1, idx + 1, iter + 1, loss.item(), loss_proto.item(), adversarial_loss.item()))
                epoch_loss['total'].append(loss.item())
                epoch_loss['proto'].append(loss_proto.item())
                epoch_loss['adversarial_loss'].append((adversarial_loss.item()))
                epoch_weights['image'] = img_w
                epoch_weights['text'] = txt_w
                epoch_weights['image_discrim'] = img_D
                epoch_weights['text_discrim'] = txt_D
            ...
            epoch_loss['adversarial_loss'] = sum(epoch_loss['adversarial_loss']) / len(epoch_loss['adversarial_loss'])
            local_losses.append(copy.deepcopy(epoch_loss['total']))
            local_proto_losses.append(copy.deepcopy(epoch_loss['proto']))
            local_adversarial_losses.append(copy.deepcopy(epoch_loss['adversarial_loss']))
            ...
            img_discrim_diff = OrderedDict({
                k: p1 - p0
                for (k, p1), p0 in zip(
                    epoch_weights['image_discrim'].items(),
                    img_discrim_frz_model_params.values(), )})
            txt_discrim_diff = OrderedDict({
                k: p1 - p0
                for (k, p1), p0 in zip(
                    epoch_weights['text_discrim'].items(),
                    txt_discrim_frz_model_params.values(), )})
            ...
            #update discriminator hypernetwork
            img_discrim_hypernet = update_hypernetwork(img_discrim_hypernet, img_discrimnator_params_list, idx, img_discrim_diff, opt)
            txt_discrim_hypernet = update_hypernetwork(txt_discrim_hypernet, txt_discrimnator_params_list, idx, txt_discrim_diff, opt)
            #update discriminator 
            img_discrim_updated_params = []
            for param, diff in zip(
                    img_discrimnator_params_list[idx], img_discrim_diff.values()
            ):
                img_discrim_updated_params.append((param + diff).detach())
            img_discrimnator_params_list[idx] = img_discrim_updated_params

            txt_discrim_updated_params = []
            for param, diff in zip(
                    txt_discrimnator_params_list[idx], txt_discrim_diff.values()
            ):
                txt_discrim_updated_params.append((param + diff).detach())
            txt_discrimnator_params_list[idx] = txt_discrim_updated_params
            del img_model_net, txt_model_net, img_discrim_net, txt_discrim_net,
            gc.collect()
    
        # calculate avg clients' losses as global
        loss_avg = sum(local_losses) / len(local_losses)
        loss_proto_avg = sum(local_proto_losses) / len(local_proto_losses)
        loss_adversarial_avg = sum(local_adversarial_losses) / len(local_adversarial_losses)
        print('Global loss...global round: %3d, loss: %3.3f, proto loss: %3.3f, loss_adversarial_avg: %3.3f' % (round + 1, loss_avg, loss_proto_avg, loss_adversarial_avg))
        result['loss'].append(float(loss_avg))
        result['proto_loss'].append(float(loss_proto_avg))
        result['adversarial_loss'].append(float(loss_adversarial_avg))
        #compute cluster prototype and unbiased prototype
        clustered_img_protos, unbiased_img_protos = proto_aggregation(local_img_protos)
        clustered_txt_protos, unbiased_txt_protos = proto_aggregation(local_txt_protos)
        ...
        if opt.valid:
            for idx in range(opt.num_users // 3):
                ...
                #mapi2t, cum_preci2t, cum_recalli2t, mapt2i, cum_prect2i, cum_recallt2i = PR_line(
                 #   img_model, txt_model, query_x, retrieval_x, query_y, retrieval_y, query_L, retrieval_L, opt.database_size)
    ...
    img_discrim_hypernet.clean_models()
    txt_discrim_hypernet.clean_models()
...
def train(**kwargs):
    ...
    img_discrimnator_params_list = []
    txt_discrimnator_params_list = []
    ...
    img_discrimnator = Discriminator()
    txt_discrimnator = Discriminator()
    if opt.use_gpu:
        img_model = img_model.cuda()
        txt_model = txt_model.cuda()
        img_discrimnator = img_discrimnator.cuda()
        txt_discrimnator = txt_discrimnator.cuda()
    ...
    img_discrimnator_params_name = [name for name in img_discrimnator.state_dict().keys()]
    txt_discrimnator_params_name = [name for name in txt_discrimnator.state_dict().keys()]
    ...
    for client in range(opt.num_users):
        ...
        img_discrimnator_params_list.append(list(img_discrimnator.state_dict().values()))
        txt_discrimnator_params_list.append(list(txt_discrimnator.state_dict().values()))
    ...
    FedProto_taskheter(opt, X, Y, L, user_groups, img_model, txt_model, img_discrimnator, txt_discrimnator, S,
...
def PR_line(img_model, txt_model, query_x, retrieval_x, query_y, retrieval_y, query_L, retrieval_L,topK):
    qBX = generate_image_code(img_model, query_x, opt.bit)
    qBY = generate_text_code(txt_model, query_y, opt.bit)
    rBX = generate_image_code(img_model, retrieval_x, opt.bit)
    rBY = generate_text_code(txt_model, retrieval_y, opt.bit)
    # save_path = os.path.join("/root/victoria/PLFedCMH-Adersarial", f'CPFedCMH_nonIDD_unequal_{opt.dataset}_{opt.bit}bits')
    # qBX = qBX.to('cpu').numpy()
    # qBY = qBY.to('cpu').numpy()
    # rBX = rBX.to('cpu').numpy()
    # rBY = rBY.to('cpu').numpy()
    # os.makedirs(save_path, exist_ok=True)
    # print("save in ", save_path)
    # np.save(os.path.join(save_path, "qBX_and_qBY_label.npy"), query_L.numpy())
    # np.save(os.path.join(save_path, "qBX_binary.npy"), qBX)
    # np.save(os.path.join(save_path, "qBY_binary.npy"), qBY)
    # np.save(os.path.join(save_path, "rBX_and_rBY_label.npy"), retrieval_L.numpy())
    # np.save(os.path.join(save_path, "rBX_binary.npy"), rBX)
    # np.save(os.path.join(save_path, "rBY_binary.npy"), rBY)
    # torch.save(img_model.state_dict(), os.path.join(save_path, "img_model.pt"))
    # torch.save(txt_model.state_dict(), os.path.join(save_path, "txt_model.pt"))
    # print("save to ", save_path)
    mapi2t, cum_preci2t, cum_recalli2t = CalcTopMapWithPR(qBX, rBY, query_L, retrieval_L, topK)
    mapt2i, cum_prect2i, cum_recallt2i = CalcTopMapWithPR(qBY, rBX, query_L, retrieval_L, topK)
    return mapi2t, cum_preci2t, cum_recalli2t, mapt2i, cum_prect2i, cum_recallt2i
...
def calc_loss(B, F, G, Y, train_L, loss_img_proto, loss_txt_proto, opt, img_adversarial_loss, txt_adversarial_loss):
    term1 = torch.sum(
    ...
    adversarial_loss = img_adversarial_loss + txt_adversarial_loss
    loss += opt.eta * loss_proto + opt.D * adversarial_loss

    return loss, loss_proto, adversarial_loss
